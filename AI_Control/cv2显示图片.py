...
def 取随机始终点():
    ...
    with open(地图线路, encoding='utf8') as f:
        ...
        while 未匹配:
            ...
            # 每个编号对应的是一条带有起点和终点的线段，下面叫做起边和终边，线路的起点和终点在边上取点
            起点边号 = 随机编号
            ...
            # 若欧氏距离小于300，则重新设置初始点，大于300则设置成功，退出循环
            if 直线距离 > 300:
                未匹配 = False
...
def cv2ImgAddText(img, text, left, top, textColor=(0, 255, 0), textSize=20):
    ...
    fontStyle = ImageFont.truetype(
        '/usr/share/fonts/truetype/fonts-guru-extra/Saab.ttf', textSize, encoding="utf-8")
        ...
        while 未接邻终点:
            ...
            for 相邻单点 in 交汇点[str(当前节点编号)]['相邻点']:
                ...
                if 相邻单点['编号'] not in 弃用点列:
                    节点距离到起点距离 = ((相邻单点['x轴'] - 交汇点[str(当前节点编号)]['x轴']) ** 2 + (
                            相邻单点['y轴'] - 交汇点[str(当前节点编号)]['y轴']) ** 2) ** 0.5
...
# summary:不断更新目标位置，求出当前位置的到终点的距离，根据行驶距离计算得分
def 求每步得分(服务器信息, 线路总数据, 目标距离, 目标坐标传入):
    ...
    if 目标距离 == None and 线路总数据['节点列'] != []:
        目标坐标 = {}
        # 求载具坐标到第一个点的欧式距离
        距离 = ((服务器信息['载具坐标']['x坐标'] - 线路总数据['节点列'][0]['x轴']) ** 2 + (
                服务器信息['载具坐标']['y坐标'] - 线路总数据['节点列'][0]['y轴']) ** 2) ** 0.5
        # 反余弦求出载具和第一个点的角度
        ...
        if 距离 >= 30:
            ...
            # 若距离小于30，可能意味车辆不用经过第一个节点就可以到达第二个节点，将第二个节点的坐标赋值给目标坐标
            if len(线路总数据['节点列']) >= 2:
