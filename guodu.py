...
class WheelTire():
    def __init__(self):
        ...
        self.Fy = 0.0
        self.Fx = 0.0
        self.LongInfluenceOnLateralSlip = 1.5
        self.MaxLongInfluenceOnLateralSlip = 0.2
    ...
    def GetTireForces(self, WheelObject, VehicleObject, Time):
        ...
        if not MathLib.NearFloat(Velocity2D, 0.0, self.LowSpeedValue):
            ...
            self.LateralSlipAngle = MathLib.GetAngleBetweenVectors(WheelObject.Wheel.WorldWheelVelocity.normalized(), WheelRightVector)*(1.0-Clamp(abs(self.LongSlip*self.LongInfluenceOnLateralSlip), 0.0, self.MaxLongInfluenceOnLateralSlip))
...
class 车轮悬挂(): #车轮悬架
    def __init__(self):
        #Constant Values
        self.高度 = 0.75
        self.弹簧刚度 = 45000.0  #弹簧刚度
        self.减振器刚度 = 1500.0   #减振器刚度
        self.最大上升 = 0.12    #最大上升
        self.最大下沉 = 1.0 #最大下沉
        self.强制应用高度 = 0.15
        self.强制横向高度 = 0.15
...
class VehicleWheel(bge.types.KX_GameObject):    
    ...
    def SetupWheelValues(self, SetupValues):
        ...
        self.Tire.LongInfluenceOnLateralSlip = SetupValues['Tire']['LongInfluenceOnLateralSlip']
        self.Tire.MaxLongInfluenceOnLateralSlip = SetupValues['Tire']['MaxLongInfluenceOnLateralSlip']
