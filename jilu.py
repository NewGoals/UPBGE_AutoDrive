        ...
        while True:
            ...
            if buf!='':
                ...
                if 图片_字节集==None:
                    ...
                    # 组包结果=组包_总(待发送数据,图片_字节集2)
                    组包结果=组包_总(待发送数据,('K').encode())
                    sock.send(组包结果)
                    if 全局字典['B按下']!=0:
                        全局字典['B按下']=0
                        全局字典['A按下']=0
...
def 截屏():
    ...
    while True:
        ...
        if 取图:
            ...
            # lock.acquire()
            lock.acquire()
            图片_字节集2 = 读取图片_到字节集('/home/fengquanli/NewDisk/python/图片/截屏{}.png'.format(str(取值)))
            lock.release()
        time.sleep(0.003)
        #print('开始1',time.time())
        try :
